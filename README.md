...
> **재작업 검토 단계:** 2026-09-11 기준으로 기존 Git 이력을 보존한 채 요구사항과 목표
> 구조를 다시 수립하고 있습니다. 기존 코드는 운영 기준이 아니라 분석 대상 프로토타입이며,
> 삭제·교체 구현은 아직 승인되지 않았습니다. 새로운 최우선 기준은
> [재작업 요구사항 기준선](docs/REWORK_BASELINE.md), 기술 제안은
> [목표 아키텍처](docs/TARGET_ARCHITECTURE.md), 기존 자산 평가는
> [현황 감사](docs/CURRENT_STATE_AUDIT.md), 변경 과정은 [작업 기록](docs/WORK_LOG.md)을
> 확인하십시오.
> Windows 프로그램은 별도 외부 배포물이 아니라, 관리자가 VPN·파일 서버 배포 프로필을
> 지정한 뒤 관리자 또는 허가된 사용자가 G-One 웹에서 직접 내려받는 방식으로 재설계합니다.
...
배포에서는 먼저 Windows 패키지를 `windows/dist/win-x64/GOne.Client.exe`에 게시해야 합니다.
