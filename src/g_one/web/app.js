...
const state={token:sessionStorage.getItem('g-one-token')||'',setupRequired:null,me:null,devices:[],support:[],audit:[],vpn:null,peers:[],workspace:null,users:[],tokens:[],roles:[],fileServers:[],deployments:[]};
const labels={overview:'개요',workspace:'워크스페이스 관리',users:'사용자 관리',tokens:'토큰 관리',roles:'권한 관리',devices:'장치',vpn:'WireGuard 관리',downloads:'클라이언트 배포',support:'원격 지원',audit:'감사 로그'};
...
async function loadDistribution(){
  try{
    const isAdmin=state.me.roles.includes('tenant_admin');
    $('#addFileServer').classList.toggle('hidden',!isAdmin);$('#addDeployment').classList.toggle('hidden',!isAdmin);
    $('#fileServersTable').closest('.panel').classList.toggle('hidden',!isAdmin);
    if(isAdmin&&!state.users.length)await loadManagement();
    if(!state.vpn)await loadVpn();
    [state.fileServers,state.deployments]=await Promise.all([isAdmin?api('/api/v1/file-servers'):Promise.resolve([]),api('/api/v1/client-deployments')]);
    $('#fileServersTable').innerHTML=state.fileServers.length?state.fileServers.map(server=>`<div class="download-card"><span class="item-icon">▤</span><div><strong>${escapeHtml(server.name)}</strong><small>\\\\${escapeHtml(server.host)} · ${server.shares.map(escapeHtml).join(', ')}</small></div><span class="status registered">사용 가능</span></div>`).join(''):empty('먼저 내부 파일 서버를 등록하세요.');
    $('#deploymentsTable').innerHTML=state.deployments.length?state.deployments.map(profile=>`<div class="download-card"><span class="item-icon">⇩</span><div><strong>${escapeHtml(profile.name)}</strong><small>revision ${profile.revision} · ${isAdmin?profile.target_subjects.map(escapeHtml).join(', '):'내 장치용'} · ${escapeHtml(profile.allowed_ips)}</small></div><div class="profile-actions"><button class="btn primary" data-download-profile="${profile.id}" data-subject="${escapeHtml(isAdmin?profile.target_subjects[0]:state.me.subject)}">다운로드</button>${isAdmin?`<button class="btn secondary" data-edit-profile="${profile.id}">편집</button><button class="danger-btn" data-delete-profile="${profile.id}">비활성화</button>`:''}</div></div>`).join(''):empty(isAdmin?'VPN·파일 서버·사용자를 선택해 배포 프로필을 게시하세요.':'할당된 Windows 프로그램이 없습니다.');
  }catch(error){showError(error);}
}
...
$('#addFileServer').onclick=()=>$('#fileServerDialog').showModal();
function openDeploymentDialog(profile=null){if(!state.vpn)return toast('먼저 VPN 서버를 설정하세요.');$('#deploymentForm').reset();$('#deploymentId').value=profile?.id||'';$('#deploymentDialogTitle').textContent=profile?'배포 프로필 편집':'Windows 배포 프로필';$('#deploymentSubmit').textContent=profile?'변경 저장':'프로필 게시';$('#deploymentVpn').innerHTML=`<option value="${state.vpn.id}">${escapeHtml(state.vpn.name)}</option>`;$('#deploymentName').value=profile?.name||'';$('#deploymentRoutes').value=profile?.allowed_ips||state.vpn.network_route;$('#deploymentServers').className='choice-list';$('#deploymentServers').innerHTML=state.fileServers.map(server=>`<label><input type="checkbox" name="deploymentServer" value="${server.id}" ${profile?.file_server_ids.includes(server.id)?'checked':''}> ${escapeHtml(server.name)} · \\${escapeHtml(server.host)}</label>`).join('');$('#deploymentUsers').className='choice-list';$('#deploymentUsers').innerHTML=state.users.filter(user=>user.status==='active').map(user=>`<label><input type="checkbox" name="deploymentUser" value="${escapeHtml(user.subject)}" ${profile?.target_subjects.includes(user.subject)?'checked':''}> ${escapeHtml(user.display_name)} · ${escapeHtml(user.subject)}</label>`).join('');$('#deploymentDialog').showModal();}
$('#addDeployment').onclick=()=>openDeploymentDialog();
$('#fileServerForm').addEventListener('submit',async event=>{event.preventDefault();const shares=$('#fileServerShares').value.split('\n').map(value=>value.trim()).filter(Boolean);try{await api('/api/v1/file-servers',{method:'POST',body:JSON.stringify({name:$('#fileServerName').value,host:$('#fileServerHost').value,shares})});event.target.reset();$('#fileServerDialog').close();toast('파일 서버를 등록했습니다.');await loadDistribution();}catch(error){showError(error);}});
$('#deploymentForm').addEventListener('submit',async event=>{event.preventDefault();const file_server_ids=$$('input[name="deploymentServer"]:checked').map(input=>input.value);const target_subjects=$$('input[name="deploymentUser"]:checked').map(input=>input.value);if(!file_server_ids.length||!target_subjects.length)return toast('파일 서버와 사용자를 하나 이상 선택하세요.');const id=$('#deploymentId').value;try{await api(id?`/api/v1/client-deployments/${id}`:'/api/v1/client-deployments',{method:id?'PUT':'POST',body:JSON.stringify({name:$('#deploymentName').value,vpn_network_id:$('#deploymentVpn').value,allowed_ips:$('#deploymentRoutes').value,file_server_ids,target_subjects})});event.target.reset();$('#deploymentDialog').close();toast(id?'배포 프로필 변경을 저장했습니다.':'배포 프로필을 게시했습니다.');await loadDistribution();}catch(error){showError(error);}});
$('#deploymentsTable').addEventListener('click',async event=>{const downloadId=event.target.dataset.downloadProfile;const editId=event.target.dataset.editProfile;const deleteId=event.target.dataset.deleteProfile;try{if(downloadId){await downloadDeployment(downloadId,event.target.dataset.subject);return toast('Windows 설치 번들을 내려받았습니다.');}if(editId){const profile=state.deployments.find(item=>item.id===editId);if(profile)openDeploymentDialog(profile);return;}if(deleteId&&confirm('이 배포 프로필을 비활성화할까요?')){await api(`/api/v1/client-deployments/${deleteId}`,{method:'DELETE'});toast('배포 프로필을 비활성화했습니다.');await loadDistribution();}}catch(error){showError(error);}});
