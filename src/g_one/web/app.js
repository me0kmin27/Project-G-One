...
$('#addDeployment').onclick=()=>{if(!state.vpn)return toast('먼저 VPN 서버를 설정하세요.');if(!state.fileServers.length)return toast('먼저 파일 서버를 등록하세요.');$('#deploymentVpn').innerHTML=`<option value="${state.vpn.id}">${escapeHtml(state.vpn.name)}</option>`;$('#deploymentRoutes').value=state.vpn.network_route;$('#deploymentServers').className='choice-list';$('#deploymentServers').innerHTML=state.fileServers.map(server=>`<label><input type="checkbox" name="deploymentServer" value="${server.id}"> ${escapeHtml(server.name)} · \\${escapeHtml(server.host)}</label>`).join('');$('#deploymentUsers').className='choice-list';$('#deploymentUsers').innerHTML=state.users.filter(user=>user.status==='active').map(user=>`<label><input type="checkbox" name="deploymentUser" value="${escapeHtml(user.subject)}"> ${escapeHtml(user.display_name)} · ${escapeHtml(user.subject)}</label>`).join('');$('#deploymentDialog').showModal();};
...
$('#deploymentForm').addEventListener('submit',async event=>{event.preventDefault();const file_server_ids=$$('input[name="deploymentServer"]:checked').map(input=>input.value);const target_subjects=$$('input[name="deploymentUser"]:checked').map(input=>input.value);if(!file_server_ids.length||!target_subjects.length)return toast('파일 서버와 사용자를 하나 이상 선택하세요.');try{await api('/api/v1/client-deployments',{method:'POST',body:JSON.stringify({name:$('#deploymentName').value,vpn_network_id:$('#deploymentVpn').value,allowed_ips:$('#deploymentRoutes').value,file_server_ids,target_subjects})});event.target.reset();$('#deploymentDialog').close();toast('배포 프로필을 게시했습니다.');await loadDistribution();}catch(error){showError(error);}});
$('#deploymentsTable').addEventListener('click',async event=>{const id=event.target.dataset.downloadProfile;if(!id)return;try{await downloadDeployment(id,event.target.dataset.subject);toast('Windows 설치 번들을 내려받았습니다.');}catch(error){showError(error);}});
